...
def harmony_bridge(name, proxy, private_key, from_chain, amount, max_gas, max_value):
    ...
    if len(from_data) == 0:
        logger.error(f'{name} | {log_name} | Error while looking for info from_chain')
        return False
    else:
    ...
    try:
        ...
        while True:
            ...
            if txCostInEther < max_gas:
                ...
                logger.warning(f'{name} | {address} | {log_name} | Approve gas price {txCostInEther}, {from_chain}, more than maximum')
                time.sleep(30)
        ...
        if "insufficient funds for gas * price + value" in str(Ex):
            logger.error(f'{name} | {address} | {log_name} | Not enough natives {amount} | {from_chain} \n {str(Ex)}')
            return False
    ...
    try: 
        ...
        while True:
            ...
            if human_value < max_value:
                ...
                logger.warning(f'{name} | {address} | {log_name} | Bridge price is {human_value}, {from_chain}, it is more than maximum')
                time.sleep(30)
                continue
            ...
            if txCostInEther < max_gas:
                ...
                logger.warning(f'{name} | {address} | {log_name} | BRIDGE gas price {txCostInEther}, {from_chain}, is more than maximum')
                time.sleep(30)
                continue
        ...
        status = ZBC.transaction_verification(name, transaction_hash, w3_from, log_name=log_name, text=f'Amount {amount} | {from_chain}',  logger=logger)
        if status == False:
            logger.error(f'{name} | {address} | {log_name} | Error with amount {amount} | {from_chain}')
            return False
        return True
    except Exception as Ex:
        if "insufficient funds for gas * price + value" in str(Ex):
            logger.error(f'{name} | {address} | {log_name} | Not enough natives {amount} | {from_chain} \n {str(Ex)}')
            return False
        logger.error(f'{name} | {address} | {log_name} | Amount error {amount} | {from_chain} \n {str(Ex)}')
        return False
